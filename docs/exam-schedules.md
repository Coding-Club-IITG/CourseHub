...
When every registered course is accounted for, the countdown points to the first exam that has not finished. Timetable cards are sorted by start time, with course code breaking ties. Courses with the same start time share that next-exam date.

If some dates are missing, the known cards remain visible and the missing course codes are listed. The summary says **Dates incomplete**, because an unmapped course might have an earlier exam. It does not present the first known date as a confirmed earliest countdown.
...
| Some registered courses have no confirmed dates | Show **Dates incomplete**, retain known cards and identify missing codes.    |
| Timetable or current registrations unavailable  | Explain what is unavailable and provide a retry action.                      |
| Empty registrations or explicitly no exams      | Show **No exam scheduled** and the existing no-exam illustration.            |
| Every listed exam has finished                  | Show **Exams finished**, retaining dated cards marked **Finished**.          |
| Request still loading or has failed             | Show a loading/unavailable state, never an initial zero-day countdown.       |
...
A successful response includes `status`, `period`, `timeZone`, `generatedAt`, `refreshAfterMs` and `exams`. The top-level status is `ready`, `excluded` or `unavailable`. For a ready response, `exams.midSem` and `exams.endSem` each contain their own status, `items`, `missingCourses` and `nextExam`. `nextExam` is `null` when a countdown cannot be confirmed.
...
| [`client/src/screens/dashboard/components/examcard/index.jsx`](../client/src/screens/dashboard/components/examcard/index.jsx)         | Display the server's countdown or availability state.                                         |
