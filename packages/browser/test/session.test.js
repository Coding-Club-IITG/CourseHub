...
for (const role of ["student", "admin"]) {
    test(`${role} bootstrap with only the other role signed in clears its own cache and permits a new login`, async (t) => {
        ...
        const transport = createTransport({
            ...
            role,
            fetchImpl: async (url) => {
                if (url.endsWith("/late")) { await late; return new Response("{}"); }
                return new Response(JSON.stringify(signedIn ? { _id: role + "-new", csrfToken: "new-token" } : {}), { status: signedIn ? 200 : 403 });
            },
        });
...
test("role-specific bootstraps remain independent, resource 403s retain sessions, and outages remain retryable", async (t) => {
    let adminStatus = 403;
    const sessions = Object.fromEntries(["student", "admin"].map((role) => {
        const transport = createTransport({
            baseUrl: "https://api.example.test/api/", role,
            fetchImpl: async (url) => new Response(JSON.stringify({ _id: role }), {
                status: url.endsWith("/resource") ? 403 : role === "admin" ? adminStatus : 200,
            }),
        });
        return [role, { transport, session: createSession({ transport, role, path: "bootstrap" }) }];
    }));
    ...
    assert.equal(student.session.queryClient.getQueryData(student.session.options.queryKey)._id, "student");
    adminStatus = 200;
    assert.equal((await admin.session.refresh())._id, "admin");
    await assert.rejects(admin.transport.json("resource"), { status: 403 });
    assert.equal(admin.session.queryClient.getQueryData(admin.session.options.queryKey)._id, "admin");
    adminStatus = 503;
    await assert.rejects(admin.session.refresh(), { status: 503 });
    assert.equal(admin.session.queryClient.getQueryState(admin.session.options.queryKey).status, "error");
    assert.equal(student.session.queryClient.getQueryData(student.session.options.queryKey)._id, "student");
    adminStatus = 200;
    assert.equal((await admin.session.refresh())._id, "admin");
});
