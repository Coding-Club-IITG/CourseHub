...
export const refreshCourseFromServer = async (
    dispatch,
    code,
    { yearIndex = null, folderId = null },
) => {
    if (!code) return;
    ...
    if (!fresh?.found || !Array.isArray(fresh.children)) return;
    ...
    const safeYearIndex =
        yearIndex !== null && yearIndex >= 0 && yearIndex < years.length
            ? yearIndex
            : years.length - 1;
